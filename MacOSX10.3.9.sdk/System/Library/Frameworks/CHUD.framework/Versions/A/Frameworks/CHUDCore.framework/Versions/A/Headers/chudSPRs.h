...
#ifndef ASSEMBLER 
/*!
  @header CHUD SPR Control Functions
  Use the CHUD SPR Functions to retrieve and modify the contents of any of the 
  special purpose registers.
  
  There are far too many SPR designations to list them all in this document. Look inside the header file: "/System/Library/Frameworks/CHUD.framework/Frameworks/CHUDCore.framework/Headers/chudSPRs.h."
*/

#ifndef CHUD_KEXT
/*!
 @function chudReadSPR
 @discussion Read the contents of a Special Purpose Register (SPR)
 @param dev The device containing the register to be read
 @param spr The spr to be read
 @param *val_p Storage for the contents of the SPR
 @result chudSucess , chudFailure
 */
int 	chudReadSPR( chud_device_t dev, int spr, uint32_t *val_p );
/*!
 @function chudWriteSPR
 @discussion Write data to a Special Purpose Register.  The calling thread or program must have root level system privileges.
 @param dev The device containing the register to be read
 @param spr The spr to be written to
 @param *val_p A 4-byte data value to be written
 @result chudSucess , chudFailure
 */
int	chudWriteSPR( chud_device_t dev, int spr, uint32_t *val_p );
/*!
 @function chudReadSPR64
 @discussion Read the contents of a 64-bit Special Purpose Register (SPR)
 @param dev The device containing the register to be read
 @param spr The spr to be read
 @param *val_p Storage for the contents of the SPR
 @result chudSucess , chudFailure
 */
int 	chudReadSPR64( chud_device_t dev, int spr, uint64_t *val_p );
/*!
 @function chudWriteSPR64
 @discussion Write 64-bit data to a Special Purpose Register.  The calling thread or program must have root level system privileges.
 @param dev The device containing the register to be read
 @param spr The spr to be written to
 @param *val_p A 8-byte data value to be written
 @result chudSucess , chudFailure
 */
int 	chudWriteSPR64( chud_device_t dev, int spr, uint64_t *val_p );
#endif /* !CHUD_KEXT */
#endif /* !ASSEMBLER */
