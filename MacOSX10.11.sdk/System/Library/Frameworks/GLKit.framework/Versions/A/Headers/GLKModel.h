...
/*!
 ...
 @discussion Memory backing these buffer are OpenGL buffers.  ModelIO will load index and vertex data from from a model asset directly in to the OpenGL buffer object.
 */
NS_CLASS_AVAILABLE(10_11, 9_0)
...
/*!
 ...
 @discussion Many GLKKMeshBuffers may reference the same OpenGL buffer object, but each with it's own offset.  (i.e. Many GLKKMeshBuffers may be suballocated from a single  OpenGL buffer object)
 */
...
/*!
 ...
 @discussion A single GL buffer is allocated for each zone.  Each zone could have many GLKMeshBuffers, each with it's own offset.  If a GLKMeshBufferAllocator is used, ModelIO will attempt to load all vertex and indexData of a single model into a single zone.  So although there maybe many GLKMeshBuffers for a model they will be backed with the same contigous GL buffer.
 */
...
/*!
 @method initWithMesh:submesh:
 @abstract Initialize this object with the parent's mesh and the ModelIO mesh
 @param mesh Parent GLKit mesh
 @param submesh ModelIO mesh from which to generate OpenGL objects
 @discussion Material textures will NOT be loaded when this initializer is used
 */
- (nonnull instancetype)initWithMesh:(nonnull GLKMesh*)mesh
 ...
 */
@property (nonatomic, nullable, assign) MDLMaterial *material;
...
/*!
 ...
 @param mesh ModelIO Mesh from which to create this GLKit mesh
 */
- (nonnull instancetype)initWithMesh:(nonnull MDLMesh*)mesh;

/*!
 @method meshesFromAsset:
 @abstract Initialize all meshes in a ModelIO asset
 @result An array of GLKit meshes built an asset
 @param asset ModelIO asset from which to create GLKit meshes
 */
+ (nonnull NSArray<GLKMesh*>*) meshesFromAsset:(nonnull MDLAsset *)asset;
