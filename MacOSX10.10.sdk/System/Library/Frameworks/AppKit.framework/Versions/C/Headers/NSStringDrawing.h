...
typedef NS_OPTIONS(NSUInteger, NSStringDrawingOptions) {
    NSStringDrawingTruncatesLastVisibleLine NS_ENUM_AVAILABLE_MAC(10_5) = (1 << 5), // Truncates and adds the ellipsis character to the last visible line if the text doesn't fit into the bounds specified. Ignored if NSStringDrawingUsesLineFragmentOrigin is not also set.
