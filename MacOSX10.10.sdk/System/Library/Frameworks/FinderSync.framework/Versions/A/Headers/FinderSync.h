...
/* Determine what directory the user is viewing, and what relevant items are selected, when displaying a menu or executing a command. Valid only while -menuItemsForTarget:selectedItems:menu: is building an NSMenu, or when executing an action specified in that menu. If the target or selected items are outside the extension's directories, these values will be nil.
 */
- (NSURL*)targetedURL;
...
/* To provide a toolbar item, the extension must implement toolbarItemName and toolbarItemImage. When the user clicks the item, -menuItemsForTarget:selectedItems:menu: will be called with FIExtensionToolbarMenu.
 */
