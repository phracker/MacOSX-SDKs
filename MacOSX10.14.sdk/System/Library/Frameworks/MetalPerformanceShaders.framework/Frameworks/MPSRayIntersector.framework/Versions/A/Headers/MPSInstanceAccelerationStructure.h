 ...
 *
 *     @code
 *     NSUInteger accelerationStructureIndexBufferLength = sizeof(uint32_t) * instanceCount;
 *    
 *     id <MTLBuffer> accelerationStructureIndexBuffer =
 *         [device newBufferWithLength:accelerationStructureIndexBufferLength
 *                             options:MTLResourceStorageModeManaged];
 *    
 *     memcpy(accelerationStructureIndexBuffer.contents, instances,
 *         accelerationStructureIndexBufferLength);
 *     [accelerationStructureIndexBuffer
 *         didModifyRange:NSMakeRange(0, accelerationStructureIndexBufferLength)];
 *    
 *     instanceAccel.accelerationStructureIndexBuffer = accelerationStructureIndexBuffer;
 *
 ...
 * for each instance
 */
@property (nonatomic, retain) id <MTLBuffer> _Nullable accelerationStructureIndexBuffer;

/**
 * @brief Offset, in bytes, into the instance buffer. Defaults to 0 bytes. Must be aligned to 4
 * bytes.
 */
@property (nonatomic) NSUInteger accelerationStructureIndexBufferOffset;
