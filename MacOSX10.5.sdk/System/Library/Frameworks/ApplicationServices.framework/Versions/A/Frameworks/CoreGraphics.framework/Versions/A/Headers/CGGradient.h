 ...
 * at location 0, one at location 1, and possibly additional colors
 * assigned to locations between 0 and 1.
 *
 ...
 * occurs using the components of that color space. See the documentation
 * of each creation function for more details. */

/* Return the CFTypeID for CGGradients. */

CG_EXTERN CFTypeID CGGradientGetTypeID(void) AVAILABLE_MAC_OS_X_VERSION_10_5_AND_LATER;
 ...
 * specify a color in the color space `space' (which may not be a pattern
 * or indexed color space). The number of locations is specified by
 * `count'; the number of color components is the product of `count' and
 * the number of color components of `space'. If no color is provided for 0
 * or 1, the gradient will use the color provided at the locations closest
 * to 0 and 1 for those values. */

CG_EXTERN CGGradientRef CGGradientCreateWithColorComponents(CGColorSpaceRef space, const CGFloat components[], const CGFloat locations[], size_t count) AVAILABLE_MAC_OS_X_VERSION_10_5_AND_LATER;
 ...
 * location 1, and intervening colors will be at equal intervals in
 * between. Otherwise, each location in `locations' should be a CGFloat
 * between 0 and 1; the array of locations should should contain the same
 * number of items as `colors'. If no color is provided for 0 or 1, the
 * gradient will use the color provided at the locations closest to 0 and 1
 * for those values. */

CG_EXTERN CGGradientRef CGGradientCreateWithColors(CGColorSpaceRef space, CFArrayRef colors, const CGFloat locations[]) AVAILABLE_MAC_OS_X_VERSION_10_5_AND_LATER;

/* Equivalent to `CFRetain' except that it doesn't crash (as `CFRetain'
 * does) if `gradient' is NULL. */

CG_EXTERN CGGradientRef CGGradientRetain(CGGradientRef gradient) AVAILABLE_MAC_OS_X_VERSION_10_5_AND_LATER;

/* Equivalent to `CFRelease' except that it doesn't crash (as `CFRelease'
 * does) if `gradient' is NULL. */

CG_EXTERN void CGGradientRelease(CGGradientRef gradient) AVAILABLE_MAC_OS_X_VERSION_10_5_AND_LATER;
