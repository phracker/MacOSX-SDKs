...
*/
- (AVCaptureConnection *)connectionWithMediaType:(NSString *)mediaType NS_AVAILABLE(10_7, NA);
...
*/
@property(nonatomic, readonly) NSArray *availableVideoCVPixelFormatTypes NS_AVAILABLE(10_7, NA);
...
*/
@property(nonatomic, readonly) NSArray *availableVideoCodecTypes NS_AVAILABLE(10_7, NA);
...
/*!
 ...
 @discussion
	...
	rate. The default value is kCMTimeInvalid.  This property has been deprecated.  AVCaptureConnection's videoMinFrameDuration
	should be used instead.
*/
@property(nonatomic) CMTime minFrameDuration NS_AVAILABLE_IPHONE(4_0);
...
/*!
 ...
 @discussion
	...
	delegate method. On Mac OS X, the captureOutput:didDropSampleBuffer:fromConnection: delegate method will be called
	for each frame that is dropped. When the value of this property is NO, delegates will be allowed more time to process
	old frames before new frames are discarded, but application memory usage may increase significantly as a result. The
...
/*!
 @method captureOutput:didOutputSampleBuffer:fromConnection:
 @abstract
	Gives the delegate the opportunity to inspect samples as they are received by the output and start and stop recording
	at exact times.
 ...
 @discussion
	...
	output files at an exact sample boundary. If called from within this method, the file output’s
	startRecordingToOutputFileURL:recordingDelegate: and resumeRecording methods are guaranteed to include the received
	sample buffer in the new file, whereas calls to stopRecording and pauseRecording are guaranteed to include all
	samples leading up to those in the current sample buffer in the existing file.
...
/*!
 ...
 @discussion
	...
	specify media encoding options for each track, and specify an interval at which movie fragments should be written.
*/
NS_CLASS_AVAILABLE(10_7, 4_0)
